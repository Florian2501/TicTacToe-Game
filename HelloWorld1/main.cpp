...
void PVC() {
	...
	while (true) {
		...
		if (currentPlayer == 1) {
			...
			//Computer ist dran
			cout << "Der Computer ist O.\n";
			comFields(&row, &column, Field);
		...
		if (checkDraw(Field)) {
			//Unentschieden
			cout << "Unentschieden! Keiner kann mehr gewinnen.\n";
			return;
		}
...
void PVP() {
	...
	while (true) {
		...
		if (currentPlayer == 1) {
			//Spieler ist dran
			cout << "Spieler 1 ist dran. Du bist X.\n";
			playerFields(&row, &column, Field);
			Field[row - 1][column - 1] = 'X';
			printCurrentField(Field);
			if (checkEnd(Field)) {
			...
			cout << "Spieler 2 ist dran. Du bist O.\n";
			playerFields(&row, &column, Field);
			Field[row - 1][column - 1] = 'O';
			printCurrentField(Field);
			if (checkEnd(Field)) {
...
*/ //alte Funktionen für Modusauswahl mit Redundanz
